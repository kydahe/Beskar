...
precompute_param * add_one_param(const uint8_t *sk, uint16_t nonce, uint16_t e_nonce, uint8_t *rhoprime, polyvecl mat[]){
  ...
  for (index = 0; nonce < e_nonce, index < PARAM_NUM; index++){
...
void precomputing(const uint8_t *sk, size_t param_num){
  ...
  for (i = 0; i < PARAM_NUM; i++) {
    ...
    // sk_params[param_index].precomputed[i].tmp = res[i].tmp;
  ...
  // printf("sk_params[%d].precomputed[0].sig = %s\n", param_index, sk_params[param_index].precomputed[0].sig);
                          ...
                          uint8_t group_index)
{
  ...
  // printf("sk_params[%d].precomputed[0].sig = %s\n", i, sk_params[i].precomputed[0].sig);
  ...
  for (j = group_index * GROUP_NUM; j< (group_index + 1) * GROUP_NUM; j++){
    ...
    // tmp = sk_params[i].precomputed[j].tmp;
    z = sk_params[i].precomputed[j].y;

    memset(sig, 0, *siglen);
    memcpy(sig, sk_params[i].precomputed[j].sig, K*POLYW1_PACKEDBYTES);
    // printf("i = %d, j = %d, sig = %s\n", i, j, sig);
    ...
    for(ii = 0; ii < L; ii++) {
      poly_pointwise_montgomery(&tmp, &c, &s1.vec[ii]);
      poly_invntt_tomont(&tmp);
      poly_add(&z.vec[ii], &z.vec[ii], &tmp);
      poly_reduce(&z.vec[ii]);
      if(poly_chknorm(&z.vec[ii], GAMMA1 - BETA))
        break;
    }
    if (ii != L)
      continue;
    ...
    for(ii = 0; ii < K; ii++) {
      ...
      * do not reveal secret information */
      poly_pointwise_montgomery(&tmp, &c, &s2.vec[ii]);
      poly_invntt_tomont(&tmp);
      poly_sub(&w0.vec[ii], &w0.vec[ii], &tmp);
      poly_reduce(&w0.vec[ii]);
      if(poly_chknorm(&w0.vec[ii], GAMMA2 - BETA))
        break;

      /* Compute hints */
      poly_pointwise_montgomery(&tmp, &c, &t0.vec[ii]);
      poly_invntt_tomont(&tmp);
      poly_reduce(&tmp);
      if(poly_chknorm(&tmp, GAMMA2))
        break;

      poly_add(&w0.vec[ii], &w0.vec[ii], &tmp);
      n = poly_make_hint(hintbuf, &w0.vec[ii], &w1.vec[ii]);
      if(pos + n > OMEGA)
        break;

      /* Store hints in signature */
      memcpy(&hint[pos], hintbuf, n);
      hint[OMEGA + ii] = pos = pos + n;
    }
    ...
    for(ii = 0; ii < L; ii++)
      polyz_pack(sig + SEEDBYTES + ii*POLYZ_PACKEDBYTES, &z.vec[ii]);

    *siglen = CRYPTO_BYTES;
    sk_params[i].precomputed[j].isUsed = true;
    // printf("Find Signatures at %d!\n", j - group_index * GROUP_NUM);
