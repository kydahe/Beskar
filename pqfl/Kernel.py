...
class Kernel:
  ...
  def runner(self, agents = [], startTime = None, stopTime = None,
    ...
    print("startTime: {}, stopTime: {}".format(startTime, stopTime))
    ...
    for sim in range(num_simulations):
      ...
      print("starting time: {}".format(s_time))
      ...
      while not self.messages.empty() and self.currentTime and (self.currentTime <= self.stopTime):
        ...
        test_messages.append(self.currentTime)
        test_messages.append(event)
      ...
      total_size = sum(sys.getsizeof(item) for item in test_messages)
      total_size_kb = total_size / 1024
      print(f"Total length: {len(test_messages)/2}, size: {total_size} bytes, or {total_size_kb} KB")
      print(type(test_messages[0]))
      # total_size = sum(len(json.dumps(item).encode('utf-8')) for item in test_messages)
      # total_size_kb = total_size / 1024
      # print(f"Total length (len-bytes): {len(test_messages)/2}, size: {total_size} bytes, or {total_size_kb} KB")
  ...
  def sendMessage(self, sender = None, recipient = None, msg = None, delay = 0, tag = None, msg_size = 10000):
    ...
    if self.agentLatencyModel is not None:
      latency = self.agentLatencyModel.get_latency(sender_id = sender, recipient_id = recipient, message_size=msg_size)
      deliverAt = sentTime + pd.Timedelta(latency)

      # Log time-in-flight if tagged.
      if tag: 
        # print(f"{tag}: {self.custom_state.get(tag, pd.Timedelta(0)).total_seconds()*1000} + {pd.Timedelta(latency).total_seconds()*1000}")
        self.custom_state[tag] = self.custom_state.get(tag, pd.Timedelta(0)) + pd.Timedelta(latency)
