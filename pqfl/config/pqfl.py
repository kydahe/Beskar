...
# Node index number starts from 1.
for i in range (0, 3):
  agents.append(NodeAgent(id = b+i,
                name = "Node Agent {}".format(i),
                type = "NodeAgent",
                iterations = num_iterations,
                num_clients = num_clients,
                num_nodes = num_nodes,
...
agent_types.extend(["NodeAgent" for i in range(0,3)])

### Configure a service agent.

agents.extend([ ServiceAgent(
                id = b+num_nodes,   # set id to be a number after all clients
                name = "Service Agent",
                type = "ServiceAgent",
                random_state = np.random.RandomState(seed=np.random.randint(low=0,high=2**32, dtype='uint64')),
                msg_fwd_delay=0,
                ...
                num_clients = num_clients,
                num_nodes = num_nodes,
                nodes = [num_clients, num_clients+1, num_clients+2],
...
print (f"Protocol Iterations: {num_iterations}, Clients: {num_clients}, Assisting Node: {num_nodes}, Agent: {agent_count}")

print ()
print(results)
print()
server_precomputed_time = (results['server_precomputed_time'].total_seconds() * 1000)
server_setup_time = (results['server_setup_time'].total_seconds() * 1000)
server_aggregate_time = (results['server_aggregate_time'].total_seconds() * 1000)
node_precomputed_time = (results['node_precomputed_time'].total_seconds() * 1000)
node_setup_time = (results['node_setup_time'].total_seconds() * 1000)
node_aggregate_time = (results['node_aggregate_time'].total_seconds() * 1000)
client_precomputed_time = (results['client_precomputed_time'].total_seconds() * 1000)
client_setup_time = (results['client_setup_time'].total_seconds() * 1000)
client_aggregate_time = (results['client_aggregate_time'].total_seconds() * 1000)

client_setup_msg = results['client_setup_msg']
client_aggregate_msg = results['client_aggregate_msg']
node_setup_msg = results['node_setup_msg']
node_aggregate_msg = results['node_aggregate_msg']
server_setup_msg = results['server_setup_msg']
server_aggregate_msg = results['server_aggregate_msg']

client_setup_comm = (results['client_setup_comm'].total_seconds() * 1000)
client_aggregate_comm = (results['client_aggregate_comm'].total_seconds() * 1000)
node_setup_comm = (results['node_setup_comm'].total_seconds() * 1000)
node_aggregate_comm = (results['node_aggregate_comm'].total_seconds() * 1000)
server_setup_comm = (results['server_setup_comm'].total_seconds() * 1000)
server_aggregate_comm = (results['server_aggregate_comm'].total_seconds() * 1000)
print ()
print (f"######## Microbenchmarks ########")
print (f"Protocol Iterations: {num_iterations}, Clients: {num_clients}, Assisting Node: {num_nodes}, Agent: {agent_count}")
print ()
...
print("Server Agent")
print (f"    Setup step:      time: {server_precomputed_time}(precomptued) + {server_setup_time}(setup) = {server_precomputed_time+server_setup_time}    msg_size: {server_setup_msg}    latency:{server_setup_comm}")
print (f"    Aggregate step:  time: {server_aggregate_time}    msg_size: {server_aggregate_msg}    latency:{server_aggregate_comm}")

print()
print("Assisting Node Agent")
print (f"    Setup step:      time: {node_precomputed_time / num_nodes}(precomptued) + {node_setup_time / num_nodes}(setup) = {node_precomputed_time / num_nodes + node_setup_time / num_nodes}    msg_size: {node_setup_msg / num_nodes}    latency:{node_setup_comm / num_nodes}")
print (f"    Aggregate step:  time: {node_aggregate_time / num_nodes}    msg_size: {node_aggregate_msg / num_nodes}    latency:{node_aggregate_comm / num_nodes}")

print()
print("Client Agent")
print (f"    Setup step:      time: {client_precomputed_time / num_clients}(precomptued) + {client_setup_time / num_clients}(setup) = {client_precomputed_time / num_clients + client_setup_time / num_clients}    msg_size: {client_setup_msg / num_clients}    latency:{client_setup_comm / num_clients}")
print (f"    Aggregate step:  time: {client_aggregate_time / num_clients}    msg_size: {client_aggregate_msg / num_clients}    latency:{client_aggregate_comm / num_clients}")

# print((time_interval.total_seconds() * 1000) + (time_interval.microseconds / 1000))
